...
function startSuggest() {
	//키보드가 눌렸을때 최초 호출되는 함수: 상태표현 전역변수를 true변경
	if (!checkFirst) {
...
function sendKeyword() {
	...
	if (keyword != '') {
		...
		sendRequest('suggest2.jsp', param, displayResult, 'POST');
	}//입력된 문자가 최소 1개 이상일때
	else {
		hide('suggest');
	}
...
function select(selectedKeyword) {//나열된 제시어 중 선택된 단어를 <input type="text" name="keyword"> 전달
...
function searchInfo() {//사원정보 검색요청
	var ename = document.searchForm.keyword.value;
	sendRequest('EmpInfo.jsp', 'searchEname=' + ename, searchResult, 'POST');
}
function searchResult() {//사원정보 검색결과 출력
